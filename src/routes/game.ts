...
const eventCards = [
  ...
  { id: 12, type: 'chance', title: '🎲 チャンス！', desc: '投資の好機！株を安く買える', effect: 0, emoji: '🎯', special: 'discount' },
  { id: 13, type: 'chance', title: '🤝 ビジネス提案', desc: '新事業への投資チャンス！', effect: 0, emoji: '💡', special: 'bonus_turn' },
...
gameRouter.post('/start', async (c) => {
  ...
  const playerName = body.playerName || 'プレイヤー'
  const playerCount = Math.min(4, Math.max(1, body.playerCount || 2))
...
gameRouter.post('/action', async (c) => {
  ...
  switch (action.type) {
    case 'buy_stock': {
      ...
      if (player.cash >= total) {
        ...
        logMsg = `📈 ${player.name}が${stock.emoji}${stock.name}を${qty}株 ¥${total.toLocaleString()}で購入！`
      } else {
    ...
    case 'sell_stock': {
      ...
      logMsg = `📉 ${player.name}が${stock.emoji}${stock.name}を${qty}株 ¥${total.toLocaleString()}で売却！`
      break
    }
    case 'buy_realestate': {
      ...
      logMsg = `🏠 ${player.name}が${re.emoji}${re.name}を ¥${re.price.toLocaleString()}で購入！`
      break
    }
    case 'end_turn': {
      logMsg = `⏭️ ${player.name}がターンを終了`
      break
    }
  }
  ...
  for (const [sid, qty] of Object.entries(player.stocks)) {
    stockValue += (newState.stockPrices[sid] || 0) * (qty as number)
  }
  let reValue = 0
  ...
  player.totalAssets = player.cash + stockValue + reValue
...
gameRouter.post('/next-turn', async (c) => {
  ...
  if (newTurn > newState.maxTurns && nextPlayer === 0) {
    ...
    const winner = newState.players.reduce((best: typeof newState.players[0], p: typeof newState.players[0]) =>
...
async function processAITurn(state: any) {
  ...
  if (rand < 0.4 && ai.cash > 10000) {
    ...
    if (affordable.length > 0) {
      ...
      logs.push(`🤖 ${ai.name}が${stock.emoji}${stock.name}を購入`)
    }
    ...
    if (available.length > 0) {
      ...
      logs.push(`🤖 ${ai.name}が${re.emoji}${re.name}を購入`)
    }
  } else if (rand < 0.6) {
    // 株を売る
    ...
    if (held.length > 0) {
      ...
      logs.push(`🤖 ${ai.name}が${stock?.emoji}${stock?.name}を売却`)
    }
  }

  // 資産再計算
  let stockValue = 0
  for (const [sid, qty] of Object.entries(ai.stocks)) {
    stockValue += (state.stockPrices[sid] || 0) * (qty as number)
  }
  let reValue = 0
  ...
  ai.totalAssets = ai.cash + stockValue + reValue
